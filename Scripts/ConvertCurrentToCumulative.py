...
def getCumulativeHospitalizations(data, window=5):
    ...
    for index in range(1, len(data) - 1):
        new_people.append(data[index] - (data[index - 1]) + new_people[(index - window) * (index >= window)] * (index >= window))
        total_hospitalizations.append(total_hospitalizations[index - 1] + new_people[index])
